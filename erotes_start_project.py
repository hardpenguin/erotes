...
def DownloadAndUnpackLove(ErotesConfig,WorkDir):
    for Platform in ErotesConfig["platforms"]:
        PlatformPath=erotes_utils.directory_utils \
        ...
        for Link in ErotesConfig["links"]:
            ...
            if Link["platform"]==Platform:
                ...
                erotes_utils.archive \
                            .ArchiveFile(PlatformPath+"/"+DownloadedPackage.Name) \
                            .Unpack()

                if "linux" in Link["platform"]:
                    erotes_utils.archive \
                                .ArchiveFile(DownloadedPackageContentsPath+"/data.tar.xz") \
                                .Unpack()
                    ...
                    for Root, Dirs, Files in os.walk(DownloadedPackageContentsPath):
                        for File in Files:
                            if ("/usr/bin" in Root) or ("/usr/lib" in Root):
                                distutils.file_util \
                                         .copy_file(Root+"/"+File,WorkDir+"/love/"+Platform)
